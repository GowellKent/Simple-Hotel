...
struct data{
    int nik;
    char nama[30],ttl[20], email[30], telp[20];
}dat[15];
...
void tambah(){
    ...
    printf("Masukkan Jumlah Data Pengunjung Yang Akan Dimasukkan (max = 15): "); scanf("%d", &jum);
        ...
        for(int i = 0 ; i < jum ; i++){
            ...
            Posisi(20,34);
            scanf("%s", &dat[i+tot].telp);
            Posisi(20,35);
            scanf(" %[^\n]%*c", &dat[i+tot].email);
        }

    snprintf(temps, sizeof(temps), "  ADMIN menambahkan %d data tamu", jum);
    append(&head, temps);
    tot += jum;
    Posisi(9,38);
    printf("*Notifikasi*");
...
void urut(){
  ...
  for(int i = 0 ; i < tot ; i++){
    ...
    Posisi(6,29);
    strcpy(temps, "  ADMIN mengurutkan data tamu hotel");
    append(&head, temps);
...
void cari(){
    ...
    snprintf(temps, sizeof(temps), "  ADMIN mencari data dari kamar No. %d", oy);
    append(&head, temps);
    Posisi(6,33);
    printf("Tekan Enter Untuk Kembali ke Menu"); getch();
}
void hapus(){
    ...
    if(temu == 1 ){
        ...
        snprintf(temps, sizeof(temps), "  ADMIN menghapus data tamu kamar No. %d", dat[x].nik);
        append(&head, temps);
...
void edit(){
    ...
    if(temu == 1 ){
            ...
            switch(jaw){
                case 1: Posisi(6,43); printf("Tanggal Baru : "); scanf(" %[^\n]%*c", &stemp);
                    strcpy(ayam, dat[x].ttl);
                    strcpy(dat[x].ttl, stemp);
                    snprintf(temps, sizeof(temps), "  ADMIN mengubah tanngal tamu kamar No. %d dari %s menjadi %s", dat[x].nik, ayam, dat[x].ttl);
                    append(&head, temps);
                    break;

                case 2: Posisi(6,43); printf("Nama Baru : "); scanf(" %[^\n]%*c", &stemp);
                    strcpy(ayam, dat[x].nama);
                    strcpy(dat[x].nama, stemp);
                    snprintf(temps, sizeof(temps), "  ADMIN mengubah nama tamu kamar No. %d dari %s menjadi %s", dat[x].nik, ayam, dat[x].nama);
                    append(&head, temps);
                    break;

                case 3: Posisi(6,43); printf("Nomor Kamar Baru : "); scanf("%d", &temp);
                    jaw = dat[x].nik;
                    dat[x].nik = temp;
                    snprintf(temps, sizeof(temps), "  ADMIN mengubah Nomor kamar tamu kamar No. %d dari %d menjadi %d", dat[x].nik, jaw, dat[x].nik);
                    append(&head, temps);
                    break;

                case 4: Posisi(6,43); printf("Nomor Telepon Baru : "); scanf(" %[^\n]%*c", &stemp);
                    strcpy(ayam, dat[x].telp);
                    strcpy(dat[x].telp, stemp);
                    snprintf(temps, sizeof(temps), "  ADMIN mengubah nomor telepon tamu kamar No. %d dari %s menjadi %s", dat[x].nik, ayam, dat[x].telp);
                    append(&head, temps);
                    break;

                case 5: Posisi(6,43); printf("Harga Baru : "); scanf(" %[^\n]%*c", &stemp);
                    strcpy(ayam, dat[x].email);
                    strcpy(dat[x].email, stemp);
                    snprintf(temps, sizeof(temps), "  ADMIN mengubah harga tamu kamar No. :%d dari %s menjadi %s", dat[x].nik, ayam, dat[x].email);
                    append(&head, temps);
                    break;
...
int posisi = 0;
    ...
    if(strlen(pw) == 0){
        printf("Password Tidak Dimasukkan");
    }else{
